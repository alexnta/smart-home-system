 ...
 * @author Alex
 */
@WebServlet(name = "GetRoomsController", urlPatterns = {"/GetRoomsController"})
public class GetRoomsController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            List<RoomDTO> list = dao.getRoomsByHomeId(homeId);
            
            // Tự tạo chuỗi JSON đơn giản (Nếu không muốn dùng thư viện Gson)
            StringBuilder json = new StringBuilder("[");
            for (int i = 0; i < list.size(); i++) {
                json.append(String.format("{\"id\":%d, \"name\":\"%s\"}", 
                            list.get(i).getRoomId(), list.get(i).getName()));
