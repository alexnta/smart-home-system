 ...
 * @author Alex
 */
@WebServlet(name = "DeleteDeviceController", urlPatterns = {"/DeleteDeviceController"})
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            if (deviceIdStr == null || deviceIdStr.trim().isEmpty()) {
                ...
                boolean success = dao.deleteDevice(deviceId);

                if (success) {
                    url = SUCCESS_PAGE;
                } else {
                    request.setAttribute("ERROR", "Failed to delete device! Device might not exist.");
                }
            }

        } catch (NumberFormatException e) {
            request.setAttribute("ERROR", "Invalid Device ID format!");
        } catch (Exception e) {
            log("Error at DeleteDeviceController: " + e.toString());
